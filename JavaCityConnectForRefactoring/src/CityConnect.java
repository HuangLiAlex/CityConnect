...
public class CityConnect {
	...
	private static int getPositionOfExistingRoute(String newStartLocation,
		...
		for (int routeNum = 0; routeNum < route.length; routeNum++) {	// use variable name routeNum instead of i

			String existing_start_location = route[routeNum][STORAGE_POSITION_START_LOCATION];
			String existing_end_location = route[routeNum][STORAGE_POSITION_END_LOCATION];
	 ...
	 */
	private static int suitableSlot(String newStartLocation, String newEndLocation) {		// use suitableSlot as method name instead of location

		for (int slots = 0; slots < route.length; slots++) {	// use slots as variable name instead of i

			String existingStartLocation = route[slots][STORAGE_POSITION_START_LOCATION];
			String existingEndLocation = route[slots][STORAGE_POSITION_END_LOCATION];
	...
	private static boolean sameRoute(String startLocation1,
		...
		if (checkNullLocation(startLocation1, endLocation1, startLocation2,
				endLocation2)) {		// improve hierarchy by extract comparison into one method
			throw new Error("Route end points cannot be null");
		}

		return sameRouteCheck(startLocation1, endLocation1, startLocation2,
				endLocation2);		// improve  hierarchy by extract same route comparison into one method
	}

	private static boolean sameRouteCheck(String startLocation1,
			String endLocation1, String startLocation2, String endLocation2) {
	...
	private static boolean checkNullLocation(String startLocation1,
			String endLocation1, String startLocation2, String endLocation2) {
		return (startLocation1 == null) || (endLocation1 == null)
				&& (startLocation2 == null) || (endLocation2 == null);
	}

	private static boolean isPositiveNonZeroInt(String distance) {	// change s into distance; change i into distanceInt
		try {
			int distanceInt = Integer.parseInt(distance);
			// return true if i is greater than 0
			return (distanceInt > 0 ? true : false);
